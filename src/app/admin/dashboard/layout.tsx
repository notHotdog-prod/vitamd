'use client';
// src/app/admin/layout.tsx (for dashboard sub-pages)
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#080B1A' }}>
      {/* Sidebar */}
      <aside className="w-64 flex-shrink-0 flex flex-col border-r border-brand-border/50"
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}
                ...
                style={active ? {
                  background: 'linear-gradient(135deg, rgba(0,229,209,0.12), rgba(26,43,204,0.12))',
                  border: '1px solid rgba(0,229,209,0.2)',
                } : {}}
              >
                <Icon size={16} className={active ? 'text-brand-cyan' : ''} />
                {item.label}
              </Link>
            );
          })}
        ...
        <div className="p-4 border-t border-brand-border/50 flex flex-col gap-2">
          <Link href="/" target="_blank"
            className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-brand-muted hover:text-white hover:bg-white/5 transition-all">
            <ExternalLink size={15} /> View Site
          </Link>
          <button onClick={handleLogout}
            className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-brand-muted hover:text-red-400 hover:bg-red-400/5 transition-all w-full">
